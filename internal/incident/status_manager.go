...
func (m *StatusManager) Create(ctx context.Context, status Status) (Status, error) {
	dbStatus := m.toDB(status)
...
func (m *StatusManager) GetByID(ctx context.Context, id StatusID) (Status, error) {
	...
	if err != nil {
		return Status{}, fmt.Errorf("get status by id: %w", err)
	}
...
func (m *StatusManager) Update(ctx context.Context, status Status) (Status, error) {
	dbStatus := m.toDB(status)
